...
export default function LedgerPage() {
  ...
  const HEAD = ["Date", "Voucher", "Particulars", "Debit", "Credit", "Balance"];
  const sheet = () => (data?.rows ?? []).map((r) =>
    [new Date(r.date).toLocaleDateString("en-GB"), r.voucherNo, r.memo || r.refType, r.debit, r.credit, r.balance]);
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {data && (
          <div className="card flex divide-x divide-line">
            <div className="px-4 py-2"><div className="text-[11px] font-semibold uppercase text-muted">Opening</div><div className="text-lg font-bold">{taka(data.opening)}</div></div>
            <div className="px-4 py-2"><div className="text-[11px] font-semibold uppercase text-muted">Closing</div><div className="text-lg font-bold">{taka(data.closing)}</div></div>
          </div>
        )}
      </div>

      <div className="flex flex-wrap items-center gap-2">
        <div className="min-w-[180px] flex-1 basis-[220px] lg:max-w-[300px]">
        ...
        <div className="flex shrink-0 gap-2">
          ...
          <button className="btn btn-ghost px-3" title="Download PDF" onClick={() => exportPdf("ledger", HEAD, sheet(), "Ledger")}><Download size={16} /></button>
      ...
      <div className="card overflow-x-auto">
        <table className="w-full min-w-[720px]">
          <thead><tr>
            <th className="th">Date</th><th className="th">Voucher</th><th className="th">Particulars</th>
            <th className="th text-right">Debit</th><th className="th text-right">Credit</th><th className="th text-right">Balance</th>
          </tr></thead>
          <tbody>
            <tr className="bg-paper">
              <td className="td font-semibold" colSpan={5}>Opening balance</td>
              <td className="td text-right font-bold">{taka(data?.opening ?? 0)}</td>
            </tr>
            {data?.rows.map((r) => (
              <tr key={r.id}>
                <td className="td text-[12px] text-muted">{dt(r.date)}</td>
                <td className="td font-mono text-[12px]">{r.voucherNo}</td>
                <td className="td">{r.memo || r.refType || "—"}</td>
            ...
            {data && data.rows.length === 0 && <tr><td colSpan={6} className="td py-10 text-center text-muted">No transactions in this period.</td></tr>}
          </tbody>
          {data && (
            <tfoot><tr className="bg-paper font-bold">
              <td className="td" colSpan={3}>Total</td>
              <td className="td text-right">{taka(data.totalDebit)}</td>
              <td className="td text-right">{taka(data.totalCredit)}</td>
              <td className="td text-right">{taka(data.closing)}</td>
            </tr></tfoot>
          )}
        </table>
      </div>
