...
// GET /api/purchase/products?q=&categoryId=&brandId=&date= → purchased qty per variant
export async function GET(req: NextRequest) {
  ...
  const items = await prisma.purchaseItem.findMany({
    where: {
      ...(date && { purchase: { createdAt: { gte: new Date(date), lte: new Date(`${date}T23:59:59`) } } }),
      variant: {
        product: {
  ...
  for (const it of items) {
    ...
    row.amount += Number(it.lineTotal);
